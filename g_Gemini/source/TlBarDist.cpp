...
//****************************************************************
  ...
  */
CTlBarDist::CTlBarDist(const QString& sName0)
{

  tlArray[1] = new CTlArray(sName0);

  QString sName = sName0+"P";
  QString filename = ":/tl/" + sName + ".tl";
  QFile iff(filename);
  ...
  if (!iff.open(QIODevice::ReadOnly | QIODevice::Text) || sName0 == "neutron" || sName0 == "proton" )
    {
      //cout << filename.toStdString() << std::endl;
      one = 1;
      return;
    }
  ifFile.flush();
...
{
  cout << "tl barrier width parameter = " << width << std::endl;
}
