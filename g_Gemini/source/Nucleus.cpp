...
//*****************************************************
void CNucleus::initializeDefaults()
{
    bStable = false;
    saddleToSciss = false;
    timeSinceSaddle = 0.;
...
void CNucleus::initialize(int iZ0, int iA0)
{
    CNuclide::init(iZ0,iA0);
    initializeDefaults();
}
...
CNucleus* CNucleus::acquire(int iZ0, int iA0)
{
    if (!recycled.empty())
    {
        CNucleus* nucleus = recycled.back();
        recycled.pop_back();
        nucleus->initialize(iZ0,iA0);
        return nucleus;
    }
...
CNucleus* CNucleus::acquire(int iZ0, int iA0, float fEx0, float fJ0)
{
    if (!recycled.empty())
    {
        CNucleus* nucleus = recycled.back();
        recycled.pop_back();
        nucleus->initialize(iZ0,iA0,fEx0,fJ0);
        return nucleus;
    }
    return new CNucleus(iZ0,iA0,fEx0,fJ0);
}

void CNucleus::release(CNucleus* nucleus)
{
    if (!nucleus)
    {
        return;
    }
    recycled.push_back(nucleus);
}
 ...
 */
void CNucleus::print(const char *str)
{
    //prints out paramters of nucleus
    // oleg  cout << strName << endl;
    if(str) cout << str <<  " ";
    cout << "=================================" << endl;
    cout << "Z= " << iZ << " A= " << iA << "  -->  " <<   strChemName << endl;
    //  cout << "mass excess = " << fExpMass << endl;
    cout << "Ex= " << fEx << " MeV;   J= " << fJ <<  "   KE= " << getKE() << " MeV" << endl;
    ...
    if (origin2 == 0) cout << " preSaddle emission";
    else if (origin2 == 1) cout << "saddle-to-scission emission";
    else if (origin2 == 2) cout << "from light  fission fragment";
    else if (origin2 == 3) cout << "from heavy  fission fragment";

    cout << "     emission time = " << timeSinceStart << " zs" << endl;
}
//WWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWW
QString CNucleus::printHtml()
{
    QString output = "" + QString::fromStdString(strName) + "<br>";
    output += "Z= " + QString::number(iZ) + " A= " + QString::number(iA) + "<br>";//endl;
    output += "mass excess = " + QString::number(fExpMass) +  "<br>";//endl;
    output +=  QString::fromStdString(strChemName) + "<br>";// << endl;
    output += "Ex= " + QString::number(fEx) + " J= " + QString::number(fJ) + "<br>";// endl;
    output += "velocity= " + QString::number(velocity[0]) + " " + QString::number(velocity[1]) + " "
              + QString::number(velocity[2]) + " cm/ns <br>";// << endl;
    output += "KE= " + QString::number(getKE()) + " MeV<br>";// << endl;
    output += "Ex = " + QString::number(fEx) + "<br>";//endl;
    output += "spin axis, theta = " + QString::number(spin.theta*180./pi) + " phi= "
              + QString::number(spin.phi*180/pi) + " deg <br>";// << endl;
    if (origin2 == 0) output += " preSaddle emission<br>";// << endl;
    else if (origin2 == 1) output += "saddle-to-scission emission<br>";// << endl;
    else if (origin2 == 2) output += "from light  fission fragment<br>";// << endl;
    else if (origin2 == 3) output += "from heavy  fission fragment<br>";// << endl;


    output += "emission time = " + QString::number(timeSinceStart) + " zs <br>";// << endl;
...
void CNucleus::binaryDecay()
{
    ...
    if (notStatistical)
    {
        daughterLight = CNucleus::acquire(evap->decay[notStatisticalMode].Z1,
                                          evap->decay[notStatisticalMode].A1);
        daughterHeavy = CNucleus::acquire(iZ-evap->decay[notStatisticalMode].Z1,
    ...
    if (fEx > 0. && (iZ == 3 || (iZ == 2 && iA == 5)))
    {
        ...
        daughterLight = CNucleus::acquire(ejectileZ,ejectileA);
        daughterHeavy = CNucleus::acquire(iZ-ejectileZ,iA-ejectileA);

        double Ek = fExpMass + fEx - daughterLight->fExpMass - daughterHeavy->fExpMass;

        //cout << "Ek = " << Ek << endl;
        if (Ek <= 0.)
        {
            release(daughterLight);
            release(daughterHeavy);
            daughterLight = NULL;
    ...
    //  cout << "Oleg widths  " << widthEvaporation << "   gamma " << widthGamma << endl;
  ...
  if (widthGamma/width > 0.99) //terminate particle evaporation.
    {
     // cout << "widthGamma/width > 0.99) //terminate particle evaporation.\n";
      bStable = 1;
      sumGammaEnergy += fEx;
      return;
    }
*/
    ...
    if (iChan == 0) //light particle evaporation
    {
        ...
        daughterLight = CNucleus::acquire(EvapZ1,EvapA1);
        daughterHeavy = CNucleus::acquire(EvapZ2,EvapA2);
        ...
        // cout << "evap->decay[EvapMode].Ek = " << evap->decay[EvapMode].Ek << endl;
    ...
    else if (iChan == 1) //complex fragment or asymmetric fission decay
    {
        ...
        daughterLight = CNucleus::acquire(fissionZ,fissionA);
        daughterHeavy = CNucleus::acquire(iZ-fissionZ,iA-fissionA);
...
{
    notStatistical = true;
    daughterLight = CNucleus::acquire(2,4);
    daughterHeavy = CNucleus::acquire(2,4);
    daughterLight->origin = origin;
    daughterHeavy->origin = origin;
    daughterLight->origin2 = origin2;
    daughterHeavy->origin2 = origin2;
...
{
    notStatistical = true;
    daughterLight = CNucleus::acquire(1,1);
    daughterHeavy = CNucleus::acquire(2,4);
    daughterLight->origin = origin;
    daughterHeavy->origin = origin;
    daughterLight->origin2 = origin2;
...
{
    notStatistical = true;
    daughterLight = CNucleus::acquire(0,1);
    daughterHeavy = CNucleus::acquire(2,4);
    daughterLight->origin = origin;
    daughterHeavy->origin = origin;
    daughterLight->origin2 = origin2;
...
void CNucleus::force9B()
{
    notStatistical = true;
    daughterLight = CNucleus::acquire(1,1);
    daughterHeavy = CNucleus::acquire(4,8);
    daughterLight->origin = origin;
    daughterHeavy->origin = origin;
    daughterLight->origin2 = origin2;
    daughterHeavy->origin2 = origin2;
...
{
    vector<CNucleus*> stack;
    stack.reserve(64);
    stack.push_back(this);

    while (!stack.empty())
    {
        CNucleus* current = stack.back();
        stack.pop_back();

        if (!current->bStable)
        {
            if (current->saddleToSciss && current->fEx > 0.)
            {
                current->saddleToScission();
            }
            else
            {
                current->binaryDecay();
        ...
        if (current->bStable)
        {
            ...
            if (current->iZ==4 && current->iA == 8) current->force8Be();
            else if (current->iZ==5 && current->iA == 9) current->force9B();
            else if (current->iZ == 3 && current->iA == 5) current->force5Li();
            else if (current->iZ == 2 && current->iA == 5) current->force5He();
            else
            {
                stableProducts.push_back(current);
                continue;
            }
        }

        if (current->daughterHeavy)
        {
            stack.push_back(current->daughterHeavy);
        }
        if (current->daughterLight)
        {
            stack.push_back(current->daughterLight);
        }
    }
...
void CNucleus::excite(float fEx0, float fJ0)
{
    ...
    //fShell = fExpMass - mass.getLDM(iZ,iA) - fPairing;
    if (iZ <= Zshell)
    {
        fPairing = 0.;
        fShell = 0.;
...
float CNucleus::evaporationWidth()
{
    ...
    for (int i=0;i<evap->nLight;i++)
    {
        ...
        //      cout << HF << " Oleg prob " << i <<  " "  << evap->prob[i] << "  "  << lightP->suppress <<
        //              " " << lightP->iA << " " << lightP->iZ << endl;
...
float CNucleus::getSumTl(float ek,float temp)
{
    ...
    storeSub.clear();
    int storeSubCapacity = lMax - lMin + 1;
    if (storeSubCapacity > 0)
    {
...
float CNucleus::weiskopf( bool saddle)
{
    ...
    if (lightP->residue.iZ > Zshell) lightP->fPair = mass->getPairing(lightP->residue.iZ,lightP->residue.iA);
    else lightP->fPair = 0.;
    ...
    for (;;)
    {
        ...
        //      if(lightP->iZ==0)
        //      cout <<  "=== n  " << lightP->residue.iA << " " <<
        //                U << " " <<
        //                lightP->fPair << " " <<
        //                lightP->fShell << " " <<
        //                -fJ << " " <<
        //                lightP->fMInertia << " " <<
        //                logLevelDensity2 << endl;
    ...
    //        logLevelDensity2_max << " " << logLevelDensity << " iA " << iA << " iZ " << iZ << endl;
...
void CNucleus::decay()
{
    ...
    {
        allProducts.reserve(reserveCount);
    }
    if (stableProducts.capacity() < reserveCount)
    {
        stableProducts.reserve(reserveCount);
    }
    int tries = 0;
    for (;;)
    {
