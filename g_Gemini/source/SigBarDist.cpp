...
CSigBarDist::CSigBarDist(const QString& sName0, float Zp0, float Ap0)
{
  ...
  Zp = Zp0;
  Ap = Ap0;
  sigCharged[1] = new CSigCharged(sName0,Zp,Ap);
  ...
  QString ffname = ":/tl/" + sName + ".inv";

  QFile iff(ffname);
  if(!iff.open(QIODevice::ReadOnly | QIODevice::Text) || sName0.compare("neutron")==0)
    {
      if(sName0.compare("neutron")!=0) cout << "CSigBarDist could not open" << ffname.toStdString() << endl;
      one = 1;
      return;
    }
